...
uniform float alphaMultiplier : hint_range(0, 5, 0.1) = 1.7;
...
uniform bool useCellularNoise = true;
...
void fragment() {
	...
	vec2 distortion = simplexDistortion*(vec2(simplexValue)-0.5);
	if (doubleSampleSimplex) {
		simplexValue = texture(simplexNoise, UV+distortion).r;
		distortion = simplexDistortion*(vec2(simplexValue)-0.5);
	}
	COLOR = texture(TEXTURE, UV+distortion);
	COLOR.a = min(1, COLOR.r*alphaMultiplier);
	if(useCellularNoise) {
		COLOR.a *= pow(cellular_noise((vec2(UV.y+TIME*cellularScrollSpeed, UV.x)+distortion)*cellularNoiseTiling), cellularNoiseExponent);
	}
	COLOR *= color;
